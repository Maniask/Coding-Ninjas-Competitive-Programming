...
that he has seen till now. While remembering the numbers he found that he can find some pattern in those numbers. 
...
ll murder(ll *arr, ll left, ll right){
	ll count = 0;
	if(left < right){
		ll mid = (left+right)/2;
		ll left_count = murder(arr, left, mid);
		ll right_count = murder(arr, mid+1, right);
		ll my_count = merge(arr, left, mid+1, right);
		return left_count + right_count + my_count;
	}
	return count;
}
